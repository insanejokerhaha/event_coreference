...
def rewrite(readfile,mapping,framepairs,verbframemapping,nounframemapping,adjframemapping,writing_path,union,match_framepair,lex,filt,srl_storing_path):
	...
	for index in range(len(tokenized)):
		...
		for cursor in jsonlines:
			if cursor["tokens"] == index:
				li = cursor
				frame_detected = True
				break
		if frame_detected == False:
			li = {"frames":[]}
		compiletokens = tokenized[index]
		args = getluhengresult(compiletokens)
		postags = nltk.pos_tag(compiletokens)
		stash_arglist = args
		frames = [frame_iter["target"] for frame_iter in li["frames"]]
			...
			for postag in range(len(postags)):
				if postags[postag][1].startswith("NN") and (re.sub("\W","",compiletokens[postag].lower()) in lexicon or wnl.lemmatize(re.sub("\W","",compiletokens[postag].lower())) in lexicon):
					...
					if Notredunt:
						...
						Enum = Enum + 1
		readtokenNum += len(tokenized[index])
...
if __name__ == "__main__":
	...
	fmap = open('repos/mappings/'+args.setname+'_sesamediv.map','r')
	mapping = js.loads(fmap.read())
	fmap.close()
	if args.framepair:
		fpair = open('repos/mappings/'+args.setname+'_sesameframepairs.map','r')
