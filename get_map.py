...
def expandByGraph(mappinglist):
	expandlist = list()
	#['ReferringEntry','Neutral','Perspectivized','Child','Parent']
...
def main(setnum):
						...
						for jsline in jsonlines:
							...
							for target in js_string["frames"]:
								...
								if (targ_start >= linestart and targ_start < lineend) or (targ_end > linestart and targ_end <= lineend):
									...
									if target["target"]['name'] == 'Locative_relation':
										print(file,x)
									if postags[target["target"]["spans"][0]['start']][1].startswith("VB"):
										vbframelist.append(target["target"]['name'])
									elif postags[target["target"]["spans"][0]['start']][1].startswith("NN"):
										nounframelist.append(target["target"]['name'])
									elif postags[target["target"]["spans"][0]['start']][1].startswith("JJ"):
										adjframelist.append(target["target"]['name'])
									else:
										continue
										#print('@@@@@@@')
										#print(x)
										#print(postags[target["target"]["spans"][0]['start']][1])
										#print(target)
										#print('@@@@@@@')
										#ss = input('sdsd')
	...
	fvn.write(js.dumps(list(expandByGraph(set(vbframelist)))))
	fvn.write("\n")
	fvn.write(js.dumps(list(expandByGraph(set(nounframelist)))))
	fvn.write("\n")
	fvn.write(js.dumps(list(expandByGraph(set(adjframelist)))))
	fvn.close()"""
